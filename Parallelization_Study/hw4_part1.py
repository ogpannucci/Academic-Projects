...
for i,nt in enumerate(num_threads):
    ...
    plt.loglog(NN, 1/NN**2, label='Quad Ref')
    plt.xlabel('Grid Size (n)')
    plt.ylabel('Error With Threadding: ' + str(i+1))
    plt.legend()
    # plt.savefig('error_part2_thread_' + str(i+1) + '.png', dpi=500, format='png', bbox_inches='tight', pad_inches=0.0)
    # plt.savefig('error_part1' + '.png', dpi=500, format='png', bbox_inches='tight', pad_inches=0.0)
...
# fpp_numeric = np.zeros(n**2)
# set_trace()
# compute_fd(210, 1, 0, fcn, fpp_numeric)
# compute_fd(6, 3, 0, fcn, fpp_numeric)
# # set_trace()
# compute_fd(6, 3, 1, fcn, fpp_numeric)
# compute_fd(6, 3, 2, fcn, fpp_numeric)
